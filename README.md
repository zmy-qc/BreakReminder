...
- 满 60 分钟居中弹深色提示卡：大数字倒数（默认 5 秒，可调）后自动进入屏保；
  点「进入休息」立即开始，点「本次跳过」跳过当次重新计时。
- 休息期间剩余时间：Windows 为屏幕底部半透明浮层；macOS 系统屏保在特权合成层、
  窗口无法覆盖，改为系统通知每分钟推送「还剩 X 分钟」（首次需在授权框点允许）。
- **休息（默认：系统屏保）**：`open` 拉起系统屏保（「系统设置 → 屏幕保护程序」
  选中的动画）。macOS 26 的系统屏保渲染在特权合成层、任何窗口都无法覆盖其上
  （实测 layer 2100 仍不可见），剩余时间以每分钟系统通知推送；设置中可选
  「内置休息画面」模式（全屏渐变光斑动画 + 屏幕中央大倒计时）。
- **到点恢复桌面**：授予「辅助功能」权限后，休息到点应用会注入一次合成鼠标
  事件自动遣散屏保（首次触发休息时会弹授权引导；app 更新重签后可能需重新勾选）；
  未授权则屏保保持到你触碰键鼠。
- **休息（Windows）**：播放系统屏保，屏幕底部半透明倒计时浮层。
- **立即结束**：休息期间检测到键鼠活动立即结束休息、恢复计时
  （开始 3 秒为防误触宽限期，忽略点"进入休息"后的手部余动）。
- **恢复**：内置画面模式到点自动关闭、桌面立即恢复；系统屏保模式人不在座时
  屏保保持，回来一碰即消失。
